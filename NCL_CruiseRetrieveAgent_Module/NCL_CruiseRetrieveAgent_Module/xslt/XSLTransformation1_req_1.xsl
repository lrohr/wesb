...
<!-- @generated mapFile="xslt/XSLTransformation1_req_1.map" md5sum="498756385bf8dad3f637f8980bf67797" version="7.6.100" -->
<!--
*****************************************************************************
...
<xsl:stylesheet
  ...
  <xsl:template match="/">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io4:smo" mode="map:XSLTransformation1_req_1"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io4:smo" to "io4:smo".  -->
  <xsl:template match="io4:smo"  mode="map:XSLTransformation1_req_1">
    <io4:smo>
      <context>
        <correlation>
          <xsl:attribute name="xsi:type">
            ...
            <xsl:element name="io7:Warnings">
              <xsl:element name="io7:Warning">
                ...
                <xsl:attribute name="ShortText">
                  <xsl:value-of select="'Agent not found'"/>
                </xsl:attribute>
              </xsl:element>
            </xsl:element>
          </xsl:element>
