...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "p_last_agent_id" to "P_TOTALCOUNT". -->
  <xsl:template name="P_last_agent_idToP_TOTALCOUNT">
    <xsl:param name="p_last_agent_id8"/>
    <!-- ADD CUSTOM CODE HERE. -->
         <xsl:choose>
             <xsl:when test="string(number($p_last_agent_id8)) != 'NaN'">
			  <xsl:value-of select="$p_last_agent_id8"/>
		</xsl:when>
		<xsl:otherwise>
