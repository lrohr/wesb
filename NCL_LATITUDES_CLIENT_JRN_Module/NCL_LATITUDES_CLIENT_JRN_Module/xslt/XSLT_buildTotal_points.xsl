...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "in2:executeNclseaNcl_Lat_Get_JrnOutput" to "TOTAL_POINTS". -->
  <xsl:template name="ExecuteNclseaNcl_Lat_Get_JrnOutputToTOTAL_POINTS">
    <xsl:param name="executeNclseaNcl_Lat_Get_JrnOutput"/>
    <xsl:param name="p_cursor"/>
    <xsl:param name="total_points_qty"/>
    <!-- ADD CUSTOM CODE HERE. -->
        <xsl:choose>
	        <xsl:when test="(string-length( $executeNclseaNcl_Lat_Get_JrnOutput))&gt;(number('2'))">
	                <xsl:choose>
		        		<xsl:when test="(string-length( $p_cursor))&gt;(number('2'))">
		        				<xsl:value-of select="sum($total_points_qty)   "/>
						</xsl:when>
						<xsl:otherwise>
