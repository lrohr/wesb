...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "in2:executeNclseaNcl_Lat_Get_JrnOutput" to "ERROR_MSG". -->
  <xsl:template name="ExecuteNclseaNcl_Lat_Get_JrnOutputToERROR_MSG">
    ...
    <xsl:choose>
		<xsl:when test="(string-length( $executeNclseaNcl_Lat_Get_JrnOutput))&gt;(number('2'))">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of select="'Client not found'"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:when>
		<xsl:otherwise>
