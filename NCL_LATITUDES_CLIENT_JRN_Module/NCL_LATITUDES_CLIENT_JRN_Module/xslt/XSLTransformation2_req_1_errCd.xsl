...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "in2:executeNclseaNcl_Lat_Get_JrnOutput" to "ErrCode". -->
  <xsl:template name="ExecuteNclseaNcl_Lat_Get_JrnOutputToErrCode">
    <xsl:param name="executeNclseaNcl_Lat_Get_JrnOutput1"/>
    <xsl:param name="p_cursor1"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="'a123'"/>
    <!--    xsl:choose>
         
	        <xsl:when test="(string-length( $executeNclseaNcl_Lat_Get_JrnOutput1))&gt;(number('2'))">
	                <xsl:choose>
		        		<xsl:when test="(string-length( $p_cursor1))&gt;(number('2'))">
