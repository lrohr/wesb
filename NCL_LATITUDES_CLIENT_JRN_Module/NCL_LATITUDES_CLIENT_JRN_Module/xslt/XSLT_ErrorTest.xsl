...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "in2:executeNclseaNcl_Lat_Get_JrnResponse" to "ERROR_MSG". -->
  <xsl:template name="ExecuteNclseaNcl_Lat_Get_JrnResponseToERROR_MSG">
    <xsl:param name="executeNclseaNcl_Lat_Get_JrnResponse"/>
    <xsl:param name="p_client_id"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="'no data'"/>
    
     <xsl:if test="not($executeNclseaNcl_Lat_Get_JrnResponse/$p_client_id)">
    		<xsl:value-of select="'no data'"/>
	  </xsl:if>
