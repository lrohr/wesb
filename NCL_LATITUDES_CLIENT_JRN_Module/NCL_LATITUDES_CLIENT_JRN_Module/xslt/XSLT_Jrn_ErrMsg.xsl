...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "p_cursor" to "ErrMsg". -->
  <xsl:template name="P_cursorToErrMsg">
    <xsl:param name="p_cursor"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
    
    <xsl:choose>
		<xsl:when test="( count($p_cursor) > 0 )">
			  <xsl:value-of select="'data returned'"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:value-of select="'no data returned'"/>
		</xsl:otherwise>
    </xsl:choose>
