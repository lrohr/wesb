...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "in2:executeNclseaNcl_Lat_Get_JrnResponse" to "ErrMsg". -->
  <xsl:template name="ExecuteNclseaNcl_Lat_Get_JrnResponseToErrMsg">
    <xsl:param name="executeNclseaNcl_Lat_Get_JrnResponse"/>
    <xsl:param name="p_cursor"/>
    <xsl:param name="record_id"/>
    <!-- ADD CUSTOM CODE HERE. -->
       <xsl:if test="not($executeNclseaNcl_Lat_Get_JrnResponse/$p_cursor)">
    		<xsl:value-of select="'no data111'"/>
	   </xsl:if>
	   <xsl:if test=" ($executeNclseaNcl_Lat_Get_JrnResponse/$p_cursor)">
    		<xsl:value-of select="'we have cursor data'"/>
	   </xsl:if>
