...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "in2:executeNclseaNcl_Lat_Get_JrnOutput" to "ErrMsg". -->
  <xsl:template name="ExecuteNclseaNcl_Lat_Get_JrnOutputToErrMsg">
    <xsl:param name="executeNclseaNcl_Lat_Get_JrnOutput"/>
    <xsl:param name="p_cursor"/>
    <xsl:param name="record_id"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select=" concat( string-length( $executeNclseaNcl_Lat_Get_JrnOutput) ,' aa ',  string-length( $p_cursor  ), 'bb' , 
    count( $p_cursor) ,  ' CCC ', sum($p_cursor/@total_points_qty)     )   "/>
  </xsl:template>
</xsl:stylesheet>