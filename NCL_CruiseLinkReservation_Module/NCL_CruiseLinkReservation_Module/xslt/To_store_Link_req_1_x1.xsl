...
<xsl:stylesheet
  ...
  <xsl:template name="Client_idToError_code">
    ...
    <xsl:param name="p_option"/>
    <!-- ADD CUSTOM CODE HERE. 
                ($p_option =('BOOKING_INFO_ONLY')) and (string-length($client_id)=number('0'))  -->
    <xsl:if test="$p_option ='BOOKING_INFO_ONLY'">
                <xsl:value-of select="'9915'"></xsl:value-of>
              </xsl:if>
  </xsl:template>
</xsl:stylesheet>