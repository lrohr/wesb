...
<!-- @generated mapFile="xslt/MergeClientError_req_1.map" md5sum="983155666ee4ca3a37b4758a0fbbbece" version="7.6.100" -->
<!--
*****************************************************************************
*   This file has been generated by the IBM XML Mapping Editor.
*
*   Mapping file:		MergeClientError_req_1.map
*   Map declaration(s):	MergeClientError_req_1
*   Input file(s):		smo://smo/name%3Dwsdl-primary/correlationContext%3D%257Bhttp%253A%252F%252FLink_Reservation%257Dapp_context/message%3D%257Bhttp%253A%252F%252FNCL_CruiseLinkReservation_Module%252FJDBCImport1%257DexecuteSeaSw_EntitymergeU46clientmergeduplicateextendedRequestMsg/xpath%3D%252F/smo.xsd
...
<xsl:stylesheet
    ...
    xmlns:map="http://NCL_CruiseLinkReservation_Module/xslt/MergeClientError_req_1"
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="msl:datamap">
        <msl:datamap>
          <xsl:element name="dataObject">
            ...
            <xsl:call-template name="map:MergeClientError_req_12">
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io2:smo" mode="map:MergeClientError_req_1"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io2:smo" to "io2:smo".  -->
  <xsl:template match="io2:smo"  mode="map:MergeClientError_req_1">
    <io2:smo>
      <context>
        <correlation>
          <xsl:attribute name="xsi:type">
            ...
            <xsl:if test="context/correlation/io5:NCL_CruiseLinkReservationRQ/@TransactionIdentifier">
              <xsl:element name="io5:Success">
                ...
                <xsl:call-template name="LogIDToSuccess">
                  <xsl:with-param name="LogID" select="$TransactionIdentifier"/>
                </xsl:call-template>
              </xsl:element>
            </xsl:if>
            <xsl:element name="io5:Warnings">
              <xsl:element name="io5:Warning">
                ...
                <xsl:attribute name="ShortText">
                  <xsl:value-of select="'Error in Merge'"/>
                </xsl:attribute>
              </xsl:element>
            </xsl:element>
          </xsl:element>
  ...
  <xsl:template name="map:MergeClientError_req_12">
    <xsl:param name="smo"/>
    <context>
      <correlation>
        <xsl:attribute name="xsi:type">
