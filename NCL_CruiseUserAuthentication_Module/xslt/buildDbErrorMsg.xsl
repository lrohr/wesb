...
<!-- @generated mapFile="xslt/buildDbErrorMsg.map" md5sum="353fc982b465e1487537e9f63a14f35c" version="7.6.100" -->
<!--
*****************************************************************************
...
*   XSLT import(s):   	buildDbErrorMsg_custom1.xsl
*
...
<xsl:stylesheet
    ...
    xmlns:map="http://NCL_CruiseUserAuthentication_Module/xslt/buildDbErrorMsg"
    xmlns:msl="http://www.ibm.com/xmlmap"
    exclude-result-prefixes="in in2 in3 in4 in5 str set math fn regexp dyn dp exsl date map xalan msl"
    version="1.0">

  <!-- imports  -->
  <xsl:import href="buildDbErrorMsg_custom1.xsl"/>
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="msl:datamap">
        <msl:datamap>
          <xsl:element name="dataObject">
            ...
            <xsl:call-template name="map:buildDbErrorMsg2">
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io2:smo" mode="map:buildDbErrorMsg"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io2:smo" to "io2:smo".  -->
  <xsl:template match="io2:smo"  mode="map:buildDbErrorMsg">
    <io2:smo>
      <context>
        <correlation>
          <xsl:attribute name="xsi:type">
            ...
            <xsl:element name="io5:Warnings">
              <xsl:element name="io5:Warning">
                ...
                <xsl:attribute name="ShortText">
                  <xsl:value-of select="'Please try Later - Database not available'"/>
                </xsl:attribute>
              </xsl:element>
            </xsl:element>
          </xsl:element>
  ...
  <xsl:template name="map:buildDbErrorMsg2">
    <xsl:param name="smo"/>
    <context>
      <correlation>
        <xsl:attribute name="xsi:type">
