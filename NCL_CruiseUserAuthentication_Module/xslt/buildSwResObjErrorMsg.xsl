...
<!-- @generated mapFile="xslt/buildSwResObjErrorMsg.map" md5sum="f671031d864916905767485772262a74" version="7.6.100" -->
<!--
*****************************************************************************
...
*   XSLT import(s):   	buildSwResObjErrorMsg_custom.xsl
*
...
<xsl:stylesheet
    ...
    xmlns:map="http://NCL_CruiseUserAuthentication_Module/xslt/buildSwResObjErrorMsg"
    xmlns:msl="http://www.ibm.com/xmlmap"
    exclude-result-prefixes="in in2 str set math fn regexp dyn dp exsl date map xalan msl"
    version="1.0">

  <!-- imports  -->
  <xsl:import href="buildSwResObjErrorMsg_custom.xsl"/>
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="msl:datamap">
        <msl:datamap>
          <xsl:element name="dataObject">
            ...
            <xsl:call-template name="map:buildSwResObjErrorMsg2">
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io2:smo" mode="map:buildSwResObjErrorMsg"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io2:smo" to "io2:smo".  -->
  <xsl:template match="io2:smo"  mode="map:buildSwResObjErrorMsg">
    <io2:smo>
      <context>
        <correlation>
          <xsl:attribute name="xsi:type">
            ...
            <xsl:element name="io5:Warnings">
              <xsl:element name="io5:Warning">
                ...
                <xsl:attribute name="ShortText">
                  <xsl:value-of select="'Please try Later - Seaware is not available'"/>
                </xsl:attribute>
              </xsl:element>
            </xsl:element>
          </xsl:element>
  ...
  <xsl:template name="map:buildSwResObjErrorMsg2">
    <xsl:param name="smo"/>
    <context>
      <correlation>
        <xsl:attribute name="xsi:type">
