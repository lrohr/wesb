...
<!-- @generated mapFile="xslt/buildSwResObj_timeout_req_1.map" md5sum="18b683114fd21ed8a2ee4a02fc0f46b4" version="7.6.100" -->
<!--
*****************************************************************************
...
*   XSLT import(s):   	buildSwResObj_timeout_Success.xsl
*
...
<xsl:stylesheet
    ...
    xmlns:map="http://NCL_CruiseUserAuthentication_Module/xslt/buildSwResObj_timeout_req_1"
    xmlns:msl="http://www.ibm.com/xmlmap"
    exclude-result-prefixes="in in2 str set math fn regexp dyn dp exsl date map xalan msl"
    version="1.0">

  <!-- imports  -->
  <xsl:import href="buildSwResObj_timeout_Success.xsl"/>
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="msl:datamap">
        <msl:datamap>
          <xsl:element name="dataObject">
            ...
            <xsl:call-template name="map:buildSwResObj_timeout_req_12">
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io2:smo" mode="map:buildSwResObj_timeout_req_1"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io2:smo" to "io2:smo".  -->
  <xsl:template match="io2:smo"  mode="map:buildSwResObj_timeout_req_1">
    <io2:smo>
      <context>
      ...
      <xsl:element name="body">
        ...
        <xsl:element name="out:Perform_NCL_CruiseUserAuthenticationResponse">
          <xsl:element name="io5:NCL_CruiseUserAuthenticationRS">
            <!-- a structural mapping: "context/correlation/io5:NCL_CruiseUserAuthenticationRQ/@TransactionIdentifier"(StringLength1to32) to "io5:Success"(SuccessType) -->
            <xsl:if test="context/correlation/io5:NCL_CruiseUserAuthenticationRQ/@TransactionIdentifier">
              <xsl:element name="io5:Success">
                ...
                <xsl:variable name="TransactionIdentifier" select="context/correlation/io5:NCL_CruiseUserAuthenticationRQ/@TransactionIdentifier"/>
            ...
            <xsl:element name="io5:Warnings">
              <xsl:element name="io5:Warning">
                ...
                <xsl:attribute name="ShortText">
                  <xsl:value-of select="'swResObj timing out'"/>
                </xsl:attribute>
              </xsl:element>
            </xsl:element>
          </xsl:element>
  ...
  <xsl:template name="map:buildSwResObj_timeout_req_12">
    <xsl:param name="smo"/>
    <context>
    ...
    <xsl:element name="body">
      ...
      <xsl:element name="out:Perform_NCL_CruiseUserAuthenticationResponse">
        <xsl:element name="io5:NCL_CruiseUserAuthenticationRS">
          <!-- a structural mapping: "$smo/context/correlation/io5:NCL_CruiseUserAuthenticationRQ/@TransactionIdentifier"(StringLength1to32) to "io5:Success"(SuccessType) -->
          <xsl:if test="$smo/context/correlation/io5:NCL_CruiseUserAuthenticationRQ/@TransactionIdentifier">
            <xsl:element name="io5:Success">
              <!-- variables for custom code -->
              <xsl:variable name="TransactionIdentifier" select="$smo/context/correlation/io5:NCL_CruiseUserAuthenticationRQ/@TransactionIdentifier"/>
              <xsl:call-template name="TransactionIdentifierToSuccess">
                <xsl:with-param name="TransactionIdentifier" select="$TransactionIdentifier"/>
              </xsl:call-template>
            </xsl:element>
