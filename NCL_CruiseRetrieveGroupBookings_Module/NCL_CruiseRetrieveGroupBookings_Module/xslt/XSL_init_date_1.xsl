...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "group_init_date" to "GroupInitDate". -->
  <xsl:template name="Group_init_dateToGroupInitDate">
    <xsl:param name="group_init_date"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="concat(substring($group_init_date,'6','2'),'/',substring($group_init_date,'9','2'), '/', substring($group_init_date,'1','4'),  ' 00:00:00'  )"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>