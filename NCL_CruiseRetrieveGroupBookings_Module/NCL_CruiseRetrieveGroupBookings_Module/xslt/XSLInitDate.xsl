...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "group_name" to "GroupName". -->
  <xsl:template name="Group_nameToGroupName">
    <xsl:param name="group_name"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="concat(substring(Context/messageType,'1','4'),'/',substring(Context/messageType,'1','4'))"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>