...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "DEPTID" to "io7:DEPARTMENT". -->
  <xsl:template name="DEPTIDToDEPARTMENT">
    <xsl:param name="DEPTID"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="substring(substring-after($DEPTID,'0'),1,2 )"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>