...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "p_errorcode" to "ShortText". -->
  <xsl:template name="P_errorcodeToShortText">
    <xsl:param name="p_errorcode1"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
    <xsl:choose>
          <xsl:when test="  (    string-length($p_errorcode1)   > '1'   
                            ) ">
                 <xsl:value-of select=" 'Error creating client' "></xsl:value-of>
          </xsl:when>
