...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "Gender" to "p_gender". -->
  <xsl:template name="GenderToP_gender">
    <xsl:param name="Gender"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:choose>
          <xsl:when test="(string-length(   $Gender )) > ('0')">
            <xsl:value-of select="substring(translate($Gender,'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'), '1', '1' )"></xsl:value-of>
          </xsl:when>
    </xsl:choose>
