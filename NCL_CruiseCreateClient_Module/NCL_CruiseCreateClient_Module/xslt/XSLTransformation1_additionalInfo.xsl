...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "FutureCruisePlan" to "p_additional_info". -->
  <xsl:template name="FutureCruisePlanToP_additional_info">
    <xsl:param name="FutureCruisePlan"/>
    <xsl:param name="PreferredBookingMode"/>
    <xsl:param name="PreferredCruiseLength"/>
    <xsl:param name="PreferredCruiseTime"/>
    <xsl:param name="PreviouslyCruised"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
     
    
    <xsl:choose>
          <xsl:when test="  (    string-length($PreviouslyCruised)   > '0'   
                           or    string-length($FutureCruisePlan)    > '0' 
                           or  string-length($PreferredBookingMode)  > '0' 
                           or  string-length($PreferredCruiseLength) > '0' 
                           or  string-length($PreferredCruiseTime)   > '0' 
                            ) ">
