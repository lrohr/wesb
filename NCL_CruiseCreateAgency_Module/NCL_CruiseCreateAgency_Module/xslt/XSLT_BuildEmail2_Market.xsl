...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "ShareMarketInd" to "p_is_address_shipping". -->
  <xsl:template name="ShareMarketIndToP_is_address_shipping">
    <xsl:param name="ShareMarketInd"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
         <xsl:choose>
                <xsl:when test="($ShareMarketInd)=('Yes')">
