...
<xsl:stylesheet
     ...
     version="1.0">
  <!-- xsl:output method="text" encoding="UTF-8" indent="yes" xalan:indent-amount="2" -->
  <xsl:output method="text"/>
  <xsl:strip-space elements="*"/>
  <!-- The rule represents a custom mapping: "Request" to "REQ_MESSAGE_TXT". -->  
  <xsl:template name="RequestToREQ_MESSAGE_TXT" match="/">
      ...
      <xsl:for-each select="$Request/Field">
    	<xsl:choose>
    		<xsl:when test="@name = 'Cardholder_Nbr'">    		
    			<Field name="Cardholder_Nbr">yyyy</Field>
    		</xsl:when>
    		<xsl:otherwise>
