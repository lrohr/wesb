...
*   Input file(s):		smo://smo/name%3Dwsdl-primary/message%3D%257Bhttp%253A%252F%252FTP_Logger%252FJDBCOutboundInterface%257DexecuteNclseaNcl_Esb_Update_Tp_LogBGRequestMsg/xpath%3D%252F/smo.xsd
...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "record_id" to "mailContent". -->
  <xsl:template name="Record_idToMailContent">
    <xsl:param name="record_id"/>
    <xsl:param name="response_msg"/>
    <xsl:param name="failureString"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="concat('failureString: ',$failureString, ' record_id: ',$record_id,' response_msg: ',$response_msg )"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>