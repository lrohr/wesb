...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "SOURCE_CD" to "SOURCE_CD". -->
  <xsl:template name="SOURCE_CDToSOURCE_CD">
    <xsl:param name="SOURCE_CD"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
    <xsl:choose>
          <xsl:when test="  (    string-length($SOURCE_CD)   > '0'   
                          
                            ) ">
                 <xsl:value-of select=" $SOURCE_CD "></xsl:value-of>
          </xsl:when>
          <xsl:otherwise>
                   <xsl:value-of select="'NCLWEBC'"/>
            </xsl:otherwise>
  </xsl:choose>
