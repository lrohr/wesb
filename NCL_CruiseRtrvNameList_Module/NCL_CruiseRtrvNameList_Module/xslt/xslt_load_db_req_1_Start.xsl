...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "Start" to "p_sailingdate". -->
  <xsl:template name="StartToP_sailingdate">
    <xsl:param name="Start"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:choose>
       <xsl:when test="(string-length($Start))&gt;(number('2'))">
                <xsl:value-of select="concat(concat(substring($Start,number('9'),number('2')),substring($Start,number('6'),number('2'))),substring($Start,number('1'),number('4')))"></xsl:value-of>
       </xsl:when>
	</xsl:choose>
  </xsl:template>
</xsl:stylesheet>