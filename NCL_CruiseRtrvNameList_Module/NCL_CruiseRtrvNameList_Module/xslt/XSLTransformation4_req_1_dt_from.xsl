...
<xsl:stylesheet
  ...
  <xsl:template name="Sail_date_fromToStart">
    ...
    <!-- ADD CUSTOM CODE HERE. -->
     <xsl:value-of select="concat(substring($sail_date_from,number('5'),number('4')),'-',substring($sail_date_from,number('3'),number('2')),'-',substring($sail_date_from,number('1'),number('2')))"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>