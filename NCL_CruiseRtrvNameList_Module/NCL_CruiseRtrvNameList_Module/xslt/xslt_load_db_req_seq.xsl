...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "SequenceNmbr" to "p_lastresidreturned". -->
  <xsl:template name="SequenceNmbrToP_lastresidreturned">
    <xsl:param name="SequenceNmbr"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
         <xsl:choose>
            <xsl:when test="(number($SequenceNmbr))&gt;(number('0'))">
              <xsl:value-of select="$SequenceNmbr"></xsl:value-of>
            </xsl:when>
            <xsl:otherwise>
