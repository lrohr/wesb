...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "io4:NCL_CruiseRtrvNameListRQ" to "p_lastresidreturned". -->
  <xsl:template name="NCL_CruiseRtrvNameListRQToP_lastresidreturned">
    <xsl:param name="NCL_CruiseRtrvNameListRQ"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
    <xsl:choose>
            <xsl:when test="( $NCL_CruiseRtrvNameListRQ/@MoreDataEchoToken  )">
                <xsl:value-of select=" $NCL_CruiseRtrvNameListRQ/@MoreDataEchoToken  "></xsl:value-of>      
            </xsl:when>
            <xsl:otherwise>
