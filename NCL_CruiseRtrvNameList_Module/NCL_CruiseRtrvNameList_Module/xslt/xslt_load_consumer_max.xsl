...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "MaxResponses" to "p_maxrecordsoutput". -->
  <xsl:template name="MaxResponsesToP_maxrecordsoutput">
    <xsl:param name="MaxResponses"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
             <xsl:choose>
            <xsl:when test="(number($MaxResponses))&gt;(number('0'))">
              <xsl:value-of select="$MaxResponses"></xsl:value-of>
            </xsl:when>
            <xsl:otherwise>
