...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "sail_date_to" to "End". -->
  <xsl:template name="Sail_date_toToEnd">
    <xsl:param name="sail_date_to"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:value-of select="concat(
         substring($sail_date_to,number('5'),number('4')),'-',
         substring($sail_date_to,number('3'),number('2')),'-',
         substring($sail_date_to,number('1'),number('2')))"></xsl:value-of>
  </xsl:template>
</xsl:stylesheet>