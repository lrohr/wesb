...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "End" to "p_sailingenddate". -->
  <xsl:template name="EndToP_sailingenddate">
    <xsl:param name="End"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:choose>
       <xsl:when test="(string-length($End))&gt;(number('2'))">
               <xsl:value-of select="concat(concat(substring($End,number('9'),number('2')),substring($End,number('6'),number('2'))),substring($End,number('1'),number('4')))"></xsl:value-of>
       </xsl:when>
	</xsl:choose>
  </xsl:template>
</xsl:stylesheet>