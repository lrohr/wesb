...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "io4:NCL_CruiseRtrvNameListRQ" to "p_maxrecordsoutput". -->
  <xsl:template name="NCL_CruiseRtrvNameListRQToP_maxrecordsoutput">
    <xsl:param name="NCL_CruiseRtrvNameListRQ"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
    <xsl:choose>
            <xsl:when test="( $NCL_CruiseRtrvNameListRQ/@MaxResponses  )">
                <xsl:value-of select=" $NCL_CruiseRtrvNameListRQ/@MaxResponses  "></xsl:value-of>      
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="'25'"></xsl:value-of>
            </xsl:otherwise>
          </xsl:choose>
