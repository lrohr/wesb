...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "LoyaltyMembershipID" to "ErrorCode". -->
  <xsl:template name="LoyaltyMembershipIDToErrorCode">
    <xsl:param name="LoyaltyMembershipID"/>
    <!-- ADD CUSTOM CODE HERE. -->
    
    <!-- ADD CUSTOM CODE HERE. -->
      <xsl:choose>
         <xsl:when test="(lenght($LoyaltyMembershipID)>0)">
             <xsl:value-of select="'0'"></xsl:value-of>
         </xsl:when>              
         <xsl:otherwise>
           <xsl:value-of select="'9910'"></xsl:value-of>
         </xsl:otherwise>
      </xsl:choose>
