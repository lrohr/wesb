...
<xsl:stylesheet
  ...
  <!-- The rule represents a custom mapping: "ShipCode" to "p_shipcode". -->
  <xsl:template name="ShipCodeToP_shipcode">
    <xsl:param name="ShipCode"/>
    <!-- ADD CUSTOM CODE HERE. -->
    <xsl:choose>
       <xsl:when test="(string-length($ShipCode))&gt;(number('2'))">
                <xsl:value-of select="$ShipCode"></xsl:value-of>
       </xsl:when>
	</xsl:choose>
  </xsl:template>
</xsl:stylesheet>