...
<xsl:stylesheet
    ...
    xmlns:io8="http://www.ibm.com/xmlns/prod/websphere/j2ca/jdbc/nclseancl_wesb_ar_get_agency_infop_cursor"
    xmlns:io9="http://www.ibm.com/xmlns/prod/websphere/j2ca/jdbc/nclseancl_wesb_ar_get_agency_infobg"
    ...
    xmlns:io15="http://NCL_CruiseRetrieveAgency_Module"
    xmlns:io14="http://www.ibm.com/xmlns/prod/websphere/j2ca/jdbc/nclseancl_wesb_ar_get_agency_phonep_cursor"
    ...
    xmlns:map="http://NCL_CruiseRetrieveAgency_Module/xslt/xsltGetPhone"
  ...
  <xsl:template match="/">
    <xsl:choose>
      <xsl:when test="msl:datamap">
        <msl:datamap>
          <xsl:element name="dataObject">
            ...
            <xsl:call-template name="map:xsltGetPhone2">
      ...
      <xsl:otherwise>
        <xsl:apply-templates select="io21:smo" mode="map:xsltGetPhone"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- This rule represents an element mapping: "io21:smo" to "io21:smo".  -->
  <xsl:template match="io21:smo"  mode="map:xsltGetPhone">
    <io21:smo>
      <context>
        <correlation>
          <xsl:attribute name="xsi:type">
            <xsl:value-of select="'io15:appContext'"/>
          </xsl:attribute>
          ...
          <xsl:element name="address">
            <xsl:element name="Get_AgencyAddressInfo">
              <!-- a for-each transform: "p_cursor"(NclseaNcl_Wesb_Ar_Get_Agency_AddressP_Cursor) to "P_CURSOR"(<Anonymous>) -->
              <xsl:apply-templates select="body/io23:executeNclseaNcl_Wesb_Ar_Get_Agency_AddressBGResponse/io23:executeNclseaNcl_Wesb_Ar_Get_Agency_AddressBGOutput/NclseaNcl_Wesb_Ar_Get_Agency_Address/p_cursor" mode="localP_cursorToP_CURSOR_860685292"/>
            </xsl:element>
          </xsl:element>
          <xsl:element name="info">
            <xsl:element name="Get_Agency_Info">
              <!-- a for-each transform: "P_CURSOR"(<Anonymous>) to "P_CURSOR"(<Anonymous>) -->
              <xsl:apply-templates select="context/correlation/info/Get_Agency_Info/P_CURSOR" mode="localP_CURSORToP_CURSOR_535484634"/>
            </xsl:element>
          </xsl:element>
        </correlation>
      </context>
      <xsl:element name="body">
        <xsl:attribute name="xsi:type">
          <xsl:value-of select="'io4:executeNclseaNcl_Wesb_Ar_Get_Agency_PhoneBGRequestMsg'"/>
        </xsl:attribute>
        <xsl:element name="io23:executeNclseaNcl_Wesb_Ar_Get_Agency_PhoneBG">
          <xsl:element name="io23:executeNclseaNcl_Wesb_Ar_Get_Agency_PhoneBGInput">
            <xsl:element name="NclseaNcl_Wesb_Ar_Get_Agency_Phone">
  ...
  <xsl:template name="map:xsltGetPhone2">
    <xsl:param name="smo"/>
    <context>
      <correlation>
        <xsl:attribute name="xsi:type">
        ...
        <xsl:element name="address">
          <xsl:element name="Get_AgencyAddressInfo">
            <!-- a for-each transform: "p_cursor"(NclseaNcl_Wesb_Ar_Get_Agency_AddressP_Cursor) to "P_CURSOR"(<Anonymous>) -->
            <xsl:apply-templates select="$smo/body/io23:executeNclseaNcl_Wesb_Ar_Get_Agency_AddressBGResponse/io23:executeNclseaNcl_Wesb_Ar_Get_Agency_AddressBGOutput/NclseaNcl_Wesb_Ar_Get_Agency_Address/p_cursor" mode="localP_cursorToP_CURSOR_860685292"/>
          </xsl:element>
        </xsl:element>
        <xsl:element name="info">
          <xsl:element name="Get_Agency_Info">
            <!-- a for-each transform: "P_CURSOR"(<Anonymous>) to "P_CURSOR"(<Anonymous>) -->
            <xsl:apply-templates select="$smo/context/correlation/info/Get_Agency_Info/P_CURSOR" mode="localP_CURSORToP_CURSOR_535484634"/>
          </xsl:element>
        </xsl:element>
      </correlation>
    </context>
  ...
  <xsl:template match="p_cursor"  mode="localP_cursorToP_CURSOR_860685292">
    <P_CURSOR>
  ...
  <!-- This rule represents a for-each transform: "P_CURSOR" to "P_CURSOR".  -->
  <xsl:template match="P_CURSOR"  mode="localP_CURSORToP_CURSOR_535484634">
    <P_CURSOR>
      <!-- a simple data mapping: "AGENCY_ID"(decimal) to "AGENCY_ID"(decimal) -->
      <AGENCY_ID>
        <xsl:value-of select="AGENCY_ID"/>
